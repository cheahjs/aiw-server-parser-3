...
namespace Intelectix.Windows.Forms
{
    ...
    public class ListViewSortManager
    {
        ...
        {
            list.SetSortIcon(columnIndex, sortOrder);
    ...
    public class ListViewDateSort : IListViewSorter
    {
        ...
        public int Compare(String lhs, String rhs)
        {
            return DateTime.Parse(lhs, CultureInfo.CurrentCulture).
                CompareTo(DateTime.Parse(rhs, CultureInfo.CurrentCulture));
        }
    }
    ...
    public class ListViewIPSort : IListViewSorter
    {
        ...
        public int Compare(String lhs, String rhs)
        {
            IPAddress lhsip = IPAddress.Parse(lhs.Split(':')[0]);
            IPAddress rhsip = IPAddress.Parse(rhs.Split(':')[0]);
    ...
    public class ListViewPlayerSort : IListViewSorter
    {
        public int Compare(String lhs, String rhs)
        {
            Regex playerstring = new Regex("[1-9]?[0-9]/[1-9]?[0-9]\\([1-9]?[0-9]\\)");
            if (playerstring.IsMatch(lhs) && playerstring.IsMatch(rhs))
            {
                Int32 current = Int32.Parse(lhs.Split('/')[0]);
                Int32 current2 = Int32.Parse(rhs.Split('/')[0]);
                Int32 result = current - current2;
            ...
            else if (playerstring.IsMatch(lhs) && !playerstring.IsMatch(rhs))
                return -1;
            else if (!playerstring.IsMatch(lhs) && playerstring.IsMatch(rhs))
                return 1;
            else
